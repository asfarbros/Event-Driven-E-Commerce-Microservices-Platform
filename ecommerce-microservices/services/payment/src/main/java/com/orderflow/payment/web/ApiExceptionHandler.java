...
import com.orderflow.payment.razorpay.RazorpayExceptions;
...
import com.orderflow.payment.web.ApiDtos.ApiError;
...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<ApiError> paymentNotFound(PaymentNotFoundException e) {
        return respond(HttpStatus.NOT_FOUND, "payment_not_found", "No payment exists for that orderId", null);
    }

    // ---- 400 webhook --------------------------------------------------------

    @ExceptionHandler(InvalidWebhookSignatureException.class)
    public ResponseEntity<ApiError> invalidSignature(InvalidWebhookSignatureException e) {
        return respond(HttpStatus.BAD_REQUEST, "invalid_signature", "Webhook signature verification failed", null);
    }

    @ExceptionHandler(InvalidWebhookException.class)
    public ResponseEntity<ApiError> invalidWebhook(InvalidWebhookException e) {
        return respond(HttpStatus.BAD_REQUEST, "invalid_webhook", "Webhook rejected: " + e.getMessage(), null);
    }

    // ---- 409 ----------------------------------------------------------------

    @ExceptionHandler(NothingToRefundException.class)
    public ResponseEntity<ApiError> nothingToRefund(NothingToRefundException e) {
        return respond(HttpStatus.CONFLICT, "nothing_to_refund", e.getMessage(), null);
    }

    // ---- Razorpay (the external gateway) --------------------------------------

    /** We could not reach Razorpay (timeout, 5xx, circuit open). Our row records the attempt; the caller may retry. */
    @ExceptionHandler(RazorpayExceptions.Unavailable.class)
    public ResponseEntity<ApiError> gatewayUnavailable(RazorpayExceptions.Unavailable e) {
        return respond(HttpStatus.SERVICE_UNAVAILABLE, "payment_gateway_unavailable",
                "The payment gateway is temporarily unavailable. Please retry.", null);
    }

    /** Razorpay refused the request (4xx). Retrying will not help; the reason is recorded on the payment row. */
    @ExceptionHandler(RazorpayExceptions.Rejected.class)
    public ResponseEntity<ApiError> gatewayRejected(RazorpayExceptions.Rejected e) {
        return respond(HttpStatus.BAD_GATEWAY, "payment_gateway_rejected",
                "The payment gateway rejected the request" + (e.getCode() != null ? " (" + e.getCode() + ")" : "") + ".", null);
    }
    ...
    public ResponseEntity<ApiError> integrity(DataIntegrityViolationException e) {
        ...
        return respond(HttpStatus.CONFLICT, "conflict", "The request conflicts with the current payment state. Please retry.", null);
    }
    ...
    public ResponseEntity<ApiError> lockTimeout(Exception e) {
        log.warn("could not acquire payment row lock in time", kv("cause", e.getClass().getSimpleName()));
        return respond(HttpStatus.SERVICE_UNAVAILABLE, "payment_busy",
                "This payment is being updated by another request. Please retry.", null);
    }
    ...
    public ResponseEntity<ApiError> databaseDown(Exception e) {
        ...
        return respond(HttpStatus.SERVICE_UNAVAILABLE, "database_unavailable",
                "The payment database is temporarily unavailable. Please try again.", null);
    }

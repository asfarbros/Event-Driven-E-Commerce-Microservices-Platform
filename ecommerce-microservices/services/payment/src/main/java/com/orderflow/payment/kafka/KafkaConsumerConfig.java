...
/**
 * Error handling for the {@code order-events} consumer (same policy as Inventory): retry with
 * exponential backoff, then dead-letter.
 *
 * <pre>
 *   attempt 1 fails → wait initial → attempt 2 fails → wait ×multiplier → …
 *   after PAYMENT_KAFKA_RETRY_MAX_ATTEMPTS retries → record copied to the
 *   DEAD LETTER TOPIC (KAFKA_TOPIC_ORDER_EVENTS_PAYMENT_DLT), its offset is
 ...
 *   <li>Everything else (database unavailable, lock timeout, Razorpay
 *       unavailable while refunding, payment row not there yet) — retried.
 *       Retrying a refund is SAFE: the refund row's UNIQUE(payment_id) means a
 *       retry can only resume the same refund, never start a second one.</li>
 * </ul>
 *
...
public class KafkaConsumerConfig {
    ...
    @Bean
    public CommonErrorHandler kafkaErrorHandler(KafkaTemplate<String, String> kafkaTemplate, PaymentProperties properties) {
        String deadLetterTopic = properties.kafka().deadLetterTopic();
        PaymentProperties.Retry retry = properties.kafka().retry();

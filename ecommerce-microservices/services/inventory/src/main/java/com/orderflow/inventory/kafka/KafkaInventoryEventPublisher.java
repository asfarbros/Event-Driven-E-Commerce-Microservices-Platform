...
import com.orderflow.inventory.service.InventoryEventPublisher;
...
/**
 * Publishes {@link InventoryEvent}s to {@code inventory-events}.
 *
 * <p>Record layout (the contract Order Service consumes in Step 6):
 * <ul>
 *   <li><b>key</b> — {@code orderId}, so every event about one order lands on
 *       the same partition and is consumed in order.</li>
 *   <li><b>value</b> — the JSON envelope documented on {@link InventoryEvent}.</li>
 *   <li><b>headers</b> —
...
public class KafkaInventoryEventPublisher implements InventoryEventPublisher {
    ...
    public KafkaInventoryEventPublisher(KafkaTemplate<String, String> kafkaTemplate, ObjectMapper objectMapper,
                                        InventoryProperties properties) {
        this.kafkaTemplate = kafkaTemplate;
        this.objectMapper = objectMapper;
        this.topic = properties.kafka().inventoryEventsTopic();
    }
    ...
    public void publish(InventoryEvent event) {
        ...
        record.headers()
                ...
                .add(HEADER_SOURCE, bytes(InventoryEvent.SOURCE))
